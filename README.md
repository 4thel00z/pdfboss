...
Not yet supported (they error or degrade gracefully, and are on the roadmap): mesh shadings (types 4–7 and the function-based type 1) and PostScript-calculator functions · the JBIG2 features listed above · the unpainted faces listed above · group soft masks (`/SMask` in `/ExtGState`) and the non-separable blend modes (Hue, Saturation, Color, Luminosity).
