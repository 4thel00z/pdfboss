...
def annexes : List Feature := [
  ...
  { ref := .annex 'F' [3], title := "Linearized PDF Document Structure", status := .incomplete,
    note := "crates/pdfboss-core/src/linearization.rs linearization_dictionary reads the linearization parameter dictionary of Table F.1 from the first object within the first 1024 bytes (Linearized, L, the H offset and length pairs, O, E, N, T, and P with its default of 0); Document::linearization and AsyncDocument::linearization return it as written, the async document from the header window it fetches at open, and is_linearized applies the /L rule of Table F.1: a length that no longer matches the file, as after an appended update, makes the file ordinary PDF (tests linearization.rs a_document_is_linearized_only_while_the_declared_length_matches, crates/pdfboss-aio/tests/parity.rs linearization_dictionaries_agree). The CLI's info prints the result (crates/pdfboss-cli/src/main.rs info_text). Nothing acts on the dictionary: pdfboss-aio still locates objects by scanning the tail for startxref and following the xref chain (crates/pdfboss-aio/src/document.rs find_tail) instead of reading the first-page cross-reference table and first-page section it points to, and pdfboss-write emits no /Linearized dictionary or first-page section (crates/pdfboss-write/src/writer.rs writes a plain header)." },
