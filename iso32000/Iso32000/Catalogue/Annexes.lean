...
def annexes : List Feature := [
  ...
  { ref := .annex 'D' [2], title := "Latin Character Set and Encodings", status := .incomplete,
    note := "The StandardEncoding, MacRomanEncoding and WinAnsiEncoding columns exist as code-to-Unicode tables (glyph-name tables only for Standard and WinAnsi) and pdfboss-write encodes text as WinAnsi; missing are the PDFDocEncoding column, a MacRoman glyph-name table, the WinAnsi footnote mapping unused codes above 0x27 to bullet (0x81, 0x8D, 0x8F, 0x90 and 0x9D return None), and /StandardEncoding named explicitly is only honored because it falls through the default arm at font.rs." },
  { ref := .annex 'D' [3], title := "PDFDocEncoding Character Set", status := .incomplete,
    note := "decode_text_string treats a text string without a BOM as Latin-1, which matches PDFDocEncoding for 0x20 to 0x7E and 0xA1 to 0xFF but not for 0x18 to 0x1F (breve, caron and the other accents), 0x80 to 0x9F (bullet, dagger, ellipsis, curly quotes and the rest) or 0xA0 (Euro), and no PDFDocEncoding table exists for writing text strings." },
