...
Ships 12 TrueType fonts from the Croscore family: Arimo (variable-weight
regular and italic plus static bold and bold-italic), Tinos (regular, bold,
italic, bold-italic), and Cousine (regular, bold, italic, bold-italic),
plus their `OFL.txt` license and `NOTICE`. These faces are used by `pdfboss`'s renderer as substitute fonts
