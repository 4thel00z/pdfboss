...
- Structure tree: `pdfboss_core::StructureTree` (`load_with`, `place_with`, `place_items_with`, `content_items_with`, and `content_item_counts_with` for a summary count that reads no element) and `Document::content_items(&page)`, a page's marked-content sequences and tagged annotations in tree order; see [Annotations in the reading order](../guide/text.md#annotations-in-the-reading-order).
