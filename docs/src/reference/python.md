...
| `OptionalContentGroup` | One optional content group (a PDF layer): `ref`, `name`, `intent`, `usage`, `visible` under the event `Document.optional_content_groups(event)` was asked for: `"view"` (the default, also what extraction and rendering use), `"print"`, `"export"`, or `None` for the default configuration alone |
| `OptionalContentUsage` | A group's usage dictionary: `view`, `print`, `print_subtype`, `export`, `zoom_min`, `zoom_max`, `language`, `language_preferred`, `page_element`, `creator`, `creator_subtype`, `user_type`, `user_names` |
