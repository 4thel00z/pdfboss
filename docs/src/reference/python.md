...
Two more names work on existing files, neither in the table above since neither composes with `|`: `watermark(data, overlay, *, rewrite=False)` takes two PDFs as bytes and returns `data` with the first page of `overlay` drawn over every page, as an incremental update appended to `data`'s bytes, or with `rewrite=True` as a fresh, compressed file (see [Watermarking an existing file](../guide/creating.md#watermarking-an-existing-file)); `Update(doc)` stages a metadata edit over an existing `Document`, with `set_metadata(title=None, author=None, subject=None, keywords=None, creator=None, producer=None)` merging the given fields and `save(path)` or `to_bytes()` writing the result as an incremental update (see [Editing PDFs](../guide/editing.md)). Both release the GIL while they run and raise `PdfError` for an encrypted base.

Four more functions assemble documents from existing file bytes, also outside the table since none composes with `|` (see [Assembling documents](../guide/assembling.md)): `merge(inputs)` gathers each item, raw `bytes` for every page or a `(bytes, list[int])` tuple for specific 0-based pages, into one fresh document in argument order; `split(data, every)` cuts `data` into consecutive parts of `every` pages, the last part carrying whatever remains; `rotate(data, by, pages=None, rewrite=False)` turns `pages` (0-based, every page when omitted) by `by` degrees clockwise, restricted to 90, 180 or 270 (else `ValueError`), appending an incremental update by default or writing a fresh file with `rewrite=True`; `rewrite(data)` writes `data` fresh, recompressed with unreachable objects and earlier update sections left behind. `merge`, `split` and `rewrite` always produce a fresh file; only `rotate` can append instead, the same choice `Update`/`meta` offers. Every one of the four raises `PdfError` for an encrypted input, and `rotate` refuses, in either mode, a page inlined directly into `/Kids` with no object of its own, since pdfboss does not yet restructure such a page to rotate it.
