...
The twelve text faces encode as WinAnsi. A character outside the encoding is an error, never silently dropped or replaced, and the error is raised before any operator is pushed, leaving the canvas untouched. `Symbol` and `ZapfDingbats` encode through their built-in encodings (ISO 32000-1 Annex D.5 and D.6): Greek letters and mathematical signs for Symbol, the dingbats for ZapfDingbats; a character outside those tables is an encoding error, and no metrics are bundled for the two faces, so `text_width` has nothing to say about them. The library does not wrap or lay out text: one call is one line; `Standard14::text_width(text, size)` returns a string's width from the AFM metrics (bare advance widths, no kerning) for callers doing their own layout.
